...
.post-slide {
	position: relative;
	width: 32%; /* 고정값 대신 비율로 변경 */
	min-width: 300px; /* 최소값은 좀 더 작게 */
	height: 300px;
	border-radius: 12px;
	overflow: hidden;
	flex-shrink: 0;
...
.search-icon {
	position: absolute; /* 절대 위치 지정 */
	right: 10px; /* 오른쪽 여백 */
	top: calc(50% + 2px); /* 기존 중앙값에서 2px 아래로 이동 */
	transform: translateY(-50%); /* 정확한 중앙 정렬 */
	border: none; /* 버튼 테두리 제거 */
	background: none; /* 배경 제거 */
	font-size: 18px; /* 이모지 크기를 줄임 */
...
.main-container {
	...
	margin: 0 auto; /* 중앙 정렬 */
	max-width: 1200px; /* 원하는 최대 너비를 설정 (선택 사항) */
}
